...
$(document).ready(function() {
	...
	$('#logout a').click(function(e) {
		...
		//var msg = "<i class='fa fa-sign-out txt-color-orangeDark'></i> Logout <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?";
		var msg  = $.logoutMSG || "You can improve your security further after logging out by closing this opened browser";
		alertMsg.confirm(msg, function(){
...
if($.navAsAjax)
{
    ...
    $(document).on('click', 'nav a[href!="#"]', function(e) {
		...
		if (!$this.attr('target')) {
		    ...
		    if ($.root_.hasClass('mobile-view-activated')) {
			    ...
			    if (window.location.search) {
		    		address = window.location.href.replace(window.location.search, '').replace(window.location.hash, '') + '#' + $this.attr('href');
		    	} else {
		...
		var t  = '';
		if(address.indexOf('__t')>=0){
			address = address.substring(0, address.indexOf("__t=")+4) + (parseInt(address.substring(address.indexOf('__t') + 4)) + 1);
			...
			t += new Date().getTime();
			window.location.hash = address + t;
		}
...
function checkURL() {
	...
	if (url) {
		...
		var tmpUrl = url.substring(0, url.indexOf("__t")-1);
		
		$('nav li:has(a[data-children*="' + tmpUrl + '"]),nav li:has(a[href="' + tmpUrl + '"])').addClass("active");
		
		var title = ($('nav a[data-children*="' + url + '"]').attr('title'));

		// change page title from global var
		document.title = (title || '') + NUI.systemTitle;
...
function loadURL(url, container) {
	...
	window.setTimeout(function(){
		$.ajax({
			...
			beforeSend : function() {
				// cog placed
				//alert("app.js:"+DWZ.maincontainer);
				//console.debug(DWZ.maincontainer.html() + " " + DWZ.maincontainer.css("opacity"));
			...
			error : function(xhr, ajaxOptions, thrownError) {
				...
				NUI.ajaxError(xhr,ajaxOptions,thrownError);
			},
			async : false
		});
	}, 150);
...
function drawBreadCrumb() {
	...
	nav_elems.each(function() {
		...
		if (!--count) document.title = $.bread_crumb.find("li:last-child").text() + ' | ' + NUI.systemTitle;
	});
...
$(function(){
	$(".localebar").click(function(){
		$.post(basePath + 'changeLocale.do',{locale:$(this).attr("locale")},function(d){
			if(d=='ok'){
